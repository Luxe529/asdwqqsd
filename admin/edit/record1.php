...
function renderForm($one = '', $two ='', $three ='', $four ='', $five ='', $six ='', $seven ='', $eight ='', $nine ='', $error = '', $id = '')
{ ?>
<?php 
...
<body>
	...
	<section class="right" id="right">
			...
			<form action="" method="post">
				...
				<div class="input">
					<strong>Domain:</strong> <input type="text" name="domain"
					value="<?php echo $two; ?>"/>
				</div>
				<div class="input">
				...
				<div class="input">
					<strong>Twitter:</strong> <input type="text" name="twitter"
					value="<?php echo $six; ?>"/>
				</div>
				<div class="input">
					<strong>Background Image:</strong> <input type="text" name="backgroundimage"
					value="<?php echo $seven; ?>"/>
				</div>
				<div class="input">
					<strong>ReCaptcha Site Key:</strong> <input type="text" name="sitekey"
					value="<?php echo $eight; ?>"/>
				</div>
				<div class="input">
					<strong>ReCaptcha Secret Key:</strong> <input type="text" name="secretkey"
					value="<?php echo $nine; ?>"/>
				</div>
...
$backgroundimage = htmlentities($_POST['backgroundimage'], ENT_QUOTES);
$sitekey = htmlentities($_POST['sitekey'], ENT_QUOTES);
$secretkey = htmlentities($_POST['secretkey'], ENT_QUOTES);
...
renderForm($name, $domain, $logo, $color, $description, $twitter, $backgroundimage, $sitekey, $secretkey, $error, $id);
}
else
{
// if everything is fine, update the record in the database
if ($stmt = $mysqli->prepare("UPDATE siteinfo SET name = ?, domain = ?, logo = ?, color = ?, description = ?, twitter = ?, backgroundimage = ?, sitekey = ?, secretkey = ? WHERE id=?"))
{
$stmt->bind_param("sssssssssi", $name, $domain, $logo, $color, $description, $twitter, $backgroundimage, $sitekey, $secretkey, $id);
$stmt->execute();
$stmt->close();
}
...
header("Location: ../index.php?name=one");
}
}
...
if($stmt = $mysqli->prepare("SELECT * FROM siteinfo WHERE id=?"))
{
$stmt->bind_param("i", $id);
$stmt->execute();

$stmt->bind_result($id, $name, $domain, $logo, $color, $description, $twitter, $backgroundimage, $sitekey, $secretkey);
$stmt->fetch();

// show the form
renderForm($name, $domain, $logo, $color, $description, $twitter, $backgroundimage, $sitekey, $secretkey, NULL, $id);
