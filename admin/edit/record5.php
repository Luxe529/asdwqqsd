...
<body>
	...
	<section class="navbar">
		...
		<div class="r">
			<a href="<?php echo $sadmindepartment ?>/departmentut.php"><i class="fas fa-sign-out-alt"></i></a>
		</div>
	</section>
	...
	<section class="right" id="right">
			...
			<form action="" method="post">
				...
				<div class="input">
					<strong>Review:</strong> <textarea name="review"><?php echo $four; ?></textarea>
				</div>
				<div class="input">
					<strong>Rating:</strong> <input type="text" name="rating"
					value="<?php echo $five; ?>"/>
				</div>
...
*/
// if the 'id' variable is set in the department, we know that we need to edit a record
if (isset($_GET['id']))
{
// if the form's submit button is clicked, we need to process the form
if (isset($_POST['submit']))
{
// make sure the 'id' in the department is valid
if (is_numeric($_POST['id']))
{
// get variables from the department/form
$id = $_POST['id'];
...
renderForm($order1, $name, $logo, $review, $rating, $error, $id);
}
else
{
// if everything is fine, update the record in the database
if ($stmt = $mysqli->prepare("UPDATE reviews SET order1 = ?, name = ?, logo = ?, review = ?, rating = ?
WHERE id=?"))
{
$stmt->bind_param("sssssi", $order1, $name, $logo, $review, $rating, $id);
$stmt->execute();
$stmt->close();
}
...
header("Location: ../index.php?name=five");
}
}
...
if($stmt = $mysqli->prepare("SELECT * FROM reviews WHERE id=?"))
{
$stmt->bind_param("i", $id);
$stmt->execute();

$stmt->bind_result($id, $order1, $name, $logo, $review, $rating);
$stmt->fetch();

// show the form
renderForm($order1, $name, $logo, $review, $rating, NULL, $id);
...
// if they are empty, show an error message and display the form
$error = 'ERROR: Please fill in all required fields!';
renderForm($order1, $name, $logo, $review, $rating, $error);
}
else
{
// insert the new record into the database
if ($stmt = $mysqli->prepare("INSERT reviews (order1, name, logo, review, rating) VALUES (?, ?, ?, ?, ?)"))
{
$stmt->bind_param("sssss", $order1, $name, $logo, $review, $rating);
$stmt->execute();
$stmt->close();
}
// show an error if the query has an error
