...
<body>
	...
	<section class="left" id="left">
		<div class="top">
			...
			<p id="welcome">Welcome, <?php echo ucfirst($backgroundn_session) ?></p>
		</div>
		<div class="section">
			<h1>CORE</h1>
	...
	<section class="right" id="right">
			...
			<form action="" method="post">
				...
				<div class="input">
					<strong>Images:</strong> <textarea name="images"><?php echo $four; ?></textarea>
				</div>
				<div class="input">
					<strong>Features:</strong> <textarea name="features"><?php echo $five; ?></textarea>
				</div>
...
renderForm($order1, $name, $price, $images, $features, $error, $id);
}
else
{
// if everything is fine, update the record in the database
if ($stmt = $mysqli->prepare("UPDATE shop SET order1 = ?, name = ?, price = ?, images = ?, features = ? WHERE id=?"))
{
$stmt->bind_param("sssssi", $order1, $name, $price, $images, $features, $id);
$stmt->execute();
$stmt->close();
}
...
header("Location: ../index.php?name=three");
}
}
...
if($stmt = $mysqli->prepare("SELECT * FROM shop WHERE id=?"))
{
$stmt->bind_param("i", $id);
$stmt->execute();

$stmt->bind_result($id, $order1, $name, $price, $images, $features);
$stmt->fetch();

// show the form
renderForm($order1, $name, $price, $images, $features, NULL, $id);
...
// if they are empty, show an error message and display the form
$error = 'ERROR: Please fill in all required fields!';
renderForm($order1, $name, $price, $images, $features, $error);
}
else
{
// insert the new record into the database
if ($stmt = $mysqli->prepare("INSERT shop (order1, name, price, images, features) VALUES (?, ?, ?, ?, ?)"))
{
$stmt->bind_param("sssss", $order1, $name, $price, $images, $features);
$stmt->execute();
$stmt->close();
}
// show an error if the query has an error
